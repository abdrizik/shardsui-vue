...
const glyphs = useTemplateRef<HTMLElement>('glyphs')

let applied: string | null = null
let previous: Segment[] = []

watchPostEffect(() => {
  const node = glyphs.value
  if (!node || text === applied) return
...
<template>
  <span v-bind="$attrs">
    ...
    <span ref="glyphs" v-once aria-hidden="true">{{ text }}</span>
  </span>
</template>

<style scoped>
span:has(> [aria-hidden]) {
