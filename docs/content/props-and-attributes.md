...
| :-------- | :----------------------------------------- | :------ | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `as`      | `keyof HTMLElementTagNameMap \| Component` | varies  | The tag the part renders, or a component of yours to render in its place. Every part already defaults to the element correct for its role. A few parts narrow it to tags — `Field.Control` and `Combobox.Input` to `'input' \| 'textarea'`, the checkbox, radio and switch roots to `'span' \| 'button'`. |
...
A component in `as` has to render a single root element and let the attributes it receives reach it. The part keeps the semantics of the tag it documents either way, so give it a component that renders that kind of element. See [Composition](/composition).
