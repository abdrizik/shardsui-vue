...
import { isElement } from '@/internal/dom'

export type InlineRectCoords = {
  x: number
  y: number
...
type ClientRectLike = RectLike & {
  x: number
  y: number
}

type ClientRectsReference = {
...
export function getInlineRectCoords(
  trigger: Element,
  clientX: number,
  clientY: number
): InlineRectCoords | undefined {
  const { lines } = getLineRects(trigger.getClientRects())
...
function getInlineReferenceRect(
  ...
  if (coords) {
    ...
    const lineIndex = findLineIndex(lines, coords.x, coords.y)
    if (lineIndex !== -1) {
      return createClientRect(lines[lineIndex]!)
    }

    if (lines.length === 2 && lines[0]!.left > lines[1]!.right) {
      return fallback
    }
  }
...
export function createInlineMiddleware(getCoords: () => InlineRectCoords | undefined): Middleware {
  return {
    name: 'inline',
    async fn(state) {
      const reference = state.elements.reference

      if (typeof (reference as Partial<ClientRectsReference>).getClientRects !== 'function') {
        return {}
      }
      ...
      const currentCoords =
        coords?.trigger === reference || coords?.trigger === contextElement ? coords : undefined
